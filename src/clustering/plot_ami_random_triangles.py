...
def main(argv):
    ...
    for ext in list(['gcount']):
        ...
        amiplnm = path_out+'AMI_RW-Induced-'+ext+'-'+nets+'-triangles-'+'.pdf' 
        values[ext] = (irange,amirange)
        plotami(irange,amirange,amiplnm)

    #plot_combined_ami(values, nets, '%s/Combined_AMI_RW-Induced-%s-triangles.pdf' % (path_out, nets))
